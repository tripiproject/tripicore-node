...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'tripicore-node': 'https://github.com/tripiproject/tripicore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/tripicore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the tripicore-node directory and configuration
