...
var TripiRPC = require('tripid-rpc');
...
/**
 * Provides a friendly event driven API to tripid in Node.js. Manages starting and
 * stopping tripid as a child process for application support, as well as connecting
 * to multiple tripid processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Tripi(options) {
  if (!(this instanceof Tripi)) {
    return new Tripi(options);
  }
...
util.inherits(Tripi, Service);

Tripi.dependencies = [];

Tripi.DEFAULT_MAX_TXIDS = 1000;
Tripi.DEFAULT_MAX_HISTORY = 50;
Tripi.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Tripi.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Tripi.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Tripi.DEFAULT_SPAWN_RESTART_TIME = 5000;
Tripi.DEFAULT_SPAWN_STOP_TIME = 10000;
Tripi.DEFAULT_TRY_ALL_INTERVAL = 1000;
Tripi.DEFAULT_REINDEX_INTERVAL = 10000;
Tripi.DEFAULT_START_RETRY_INTERVAL = 5000;
Tripi.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Tripi.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Tripi.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'tripiuser',
  rpcpassword: 'tripipasswordkjh2335kjh',
  uacomment: 'bitcore'
};

Tripi.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Tripi.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Tripi.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Tripi.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Tripi.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Tripi.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Tripi.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Tripi.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Tripi.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Tripi.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Tripi.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Tripi.prototype._initCaches = function() {
...
Tripi.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Tripi.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Tripi.prototype.getPublishEvents = function() {
  return [
    {
...
Tripi.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'tripid/' + name, 'total:', this.subscriptions[name].length);
};

Tripi.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'tripid/' + name, 'total:', this.subscriptions[name].length);
};


Tripi.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'tripid/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Tripi.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'tripid/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Tripi.prototype.unsubscribeBalanceAll = function(emitter) {
...
Tripi.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'tripid/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Tripi.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'tripid/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Tripi.prototype.unsubscribeAddressAll = function(emitter) {
...
Tripi.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Tripi.DEFAULT_CONFIG_SETTINGS;
...
Tripi.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Tripi.prototype._expandRelativeDatadir = function() {
...
Tripi.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in tripid config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in tripid config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in tripid config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './tripi.conf');

  log.info('Using tripi config file:', configPath);
...
Tripi.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of tripicore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of tripicore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of tripicore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to tripid from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from tripid. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from tripid. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that tripid is undergoing a reindex. ' +
             ...
             'of tripicore-node services will start.');
    node._reindex = true;
  }
};

Tripi.prototype._resetCaches = function() {
...
Tripi.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Tripi.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Tripi.prototype._initChain = function(callback) {
  var self = this;
...
Tripi.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Tripi.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/tripi.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/tripi.conf';
    }
  }
  return networkPath;
};

Tripi.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Tripi.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('tripid/hashblock', message.toString('hex'));
    }
  }

};

Tripi.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Tripi.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Tripi Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Tripi.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Tripi.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Tripi.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('tripid/addressbalance', {
                    address: address,
                    txid: txid,
...
Tripi.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('tripid/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Tripi.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('tripid/rawtransaction', message.toString('hex'));
    }
...
Tripi.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Tripi.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Tripi.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Tripi.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Tripi.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockChainInfo(function(err, response) {
        ...
        log.info('Tripi Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Tripi.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Tripi.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Tripi.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/tripid.pid';
...
Tripi.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting tripi process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Tripi process unexpectedly exited with code:', code);
        log.warn('Restarting tripi child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Tripi process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn tripid.'));
      }
...
Tripi.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to tripid.'));
    }
 ...
 */
Tripi.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Tripi configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Tripi.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.syncPercentage = function(callback) {
  var self = this;
...
Tripi.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, tripicore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Tripi.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Tripi.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Tripi.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Tripi.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in tripicore is most recent to less recent
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Tripi.prototype._getConfirmationsDetail = function(transaction) {
...
Tripi.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Tripi.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Tripi.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A tripicoin transaction id
 * @param {Function} callback
 */
Tripi.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Tripi.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Tripi.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Tripi.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Tripi.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        log.error('GetRawTransaction In tripicore-node tripid.js response', response);
        log.error('GetRawTransaction In tripicore-node tripid.js txid', txid);
        log.error('GetRawTransaction In tripicore-node tripid.js error', err);
        if (err) {
          return done(self._wrapRPCError(err));
        }
...
/**
 * Will get a transaction as a Tripicore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Tripi.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getBlockChainInfo = function(callback) {
...
Tripi.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('tripid spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('tripid process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of tripi addresses to filter
 * @param {Function} callback
 */
Tripi.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Tripi.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Tripi.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Tripi.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Tripi.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Tripi.prototype.getStakingInfo = function(callback) {
    var self = this,
