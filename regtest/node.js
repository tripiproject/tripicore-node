...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.tripid.height === 150) {
            node.services.tripid.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.tripid.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('tripid/hashblock', function(data) {
        bus.unsubscribe('tripid/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.tripid.height;
      node.services.tripid.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.tripid.sendTransaction(tx.serialize(), function(err, hash) {
