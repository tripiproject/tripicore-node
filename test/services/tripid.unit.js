...
var Transaction = tripicore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/tripi.conf')));
var TripiService = proxyquire('../../lib/services/tripid', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultTripiConf = fs.readFileSync(path.resolve(__dirname, '../data/default.tripi.conf'), 'utf8');

describe('Tripi Service', function() {
	...
	var baseConfig = {
		node: {
			network: tripicore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		...
		it('will create an instance without `new`', function() {
			var tripid = TripiService(baseConfig);
			should.exist(tripid);
		});
		it('will init caches', function() {
			var tripid = new TripiService(baseConfig);

			should.exist(tripid.utxosCache);
			should.exist(tripid.txidsCache);
			should.exist(tripid.balanceCache);
			should.exist(tripid.summaryCache);
			should.exist(tripid.transactionDetailedCache);
			should.exist(tripid.accountInfo);

			should.exist(tripid.transactionCache);
			should.exist(tripid.rawTransactionCache);
			should.exist(tripid.rawJsonTransactionCache);
			should.exist(tripid.blockCache);
			should.exist(tripid.blockJsonCache);
			should.exist(tripid.blockSubsidyCache);
			should.exist(tripid.rawBlockCache);
			should.exist(tripid.blockHeaderCache);
			should.exist(tripid.zmqKnownTransactions);
			should.exist(tripid.zmqKnownBlocks);
			should.exist(tripid.lastTip);
			should.exist(tripid.lastTipTimeout);
			should.equal(tripid.dgpInfoCache, null, 'should be null');
			should.equal(tripid.miningInfoCache, null, 'should be null');
			should.equal(tripid.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(tripid.maxTxids, 1000);
			should.equal(tripid.maxTransactionHistory, 50);
			should.equal(tripid.maxAddressesQuery, 10000);
			should.equal(tripid.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(tripid.spawnRestartTime, 5000);
			should.equal(tripid.spawnStopTime, 10000);

			// try all interval
			should.equal(tripid.tryAllInterval, 1000);
			should.equal(tripid.startRetryInterval, 5000);

			// rpc limits
			should.equal(tripid.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(tripid.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var tripid = new TripiService(baseConfig);
			tripid.nodes.should.deep.equal([]);
			tripid.nodesIndex.should.equal(0);
			tripid.nodes.push({ client: sinon.stub() });
			should.exist(tripid.client);
		});
		it('will set subscriptions', function() {
			var tripid = new TripiService(baseConfig);
			tripid.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var tripid = new TripiService(baseConfig);
			tripid._initDefaults({ transactionConcurrency: 10 });
			tripid.transactionConcurrency.should.equal(10);
			tripid._initDefaults({});
			tripid.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			TripiService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var tripid = new TripiService(baseConfig);
			var methods = tripid.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var tripid = new TripiService(baseConfig);
			var events = tripid.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('tripid/rawtransaction');
			events[0].scope.should.equal(tripid);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('tripid/hashblock');
			events[1].scope.should.equal(tripid);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('tripid/addresstxid');
			events[2].scope.should.equal(tripid);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('tripid/addressbalance');
			events[3].scope.should.equal(tripid);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var tripid = new TripiService(baseConfig);
			tripid.subscribe = sinon.stub();
			tripid.unsubscribe = sinon.stub();
			var events = tripid.getPublishEvents();

			events[0].subscribe('test');
			tripid.subscribe.args[0][0].should.equal('rawtransaction');
			tripid.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			tripid.unsubscribe.args[0][0].should.equal('rawtransaction');
			tripid.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			tripid.subscribe.args[1][0].should.equal('hashblock');
			tripid.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			tripid.unsubscribe.args[1][0].should.equal('hashblock');
			tripid.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var tripid = new TripiService(baseConfig);
			var emitter = {};
			tripid.subscribe('hashblock', emitter);
			tripid.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			tripid.subscribe('rawtransaction', emitter2);
			tripid.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			tripid.subscribe('hashblock', emitter1);
			tripid.subscribe('hashblock', emitter2);
			tripid.subscribe('hashblock', emitter3);
			tripid.subscribe('hashblock', emitter4);
			tripid.subscribe('hashblock', emitter5);
			tripid.subscriptions.hashblock.length.should.equal(5);

			tripid.unsubscribe('hashblock', emitter3);
			tripid.subscriptions.hashblock.length.should.equal(4);
			tripid.subscriptions.hashblock[0].should.equal(emitter1);
			tripid.subscriptions.hashblock[1].should.equal(emitter2);
			tripid.subscriptions.hashblock[2].should.equal(emitter4);
			tripid.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var tripid = new TripiService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			tripid.subscriptions.hashblock = [emitter1];
			tripid.unsubscribe('hashblock', emitter3);
			tripid.subscriptions.hashblock.length.should.equal(1);
			tripid.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			tripid.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(tripid.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var tripid = new TripiService(baseConfig);
			var emitter = new EventEmitter();
			tripid.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var tripid = new TripiService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			tripid.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			tripid.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeAddress(emitter1);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			tripid.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(tripid, 'unsubscribeAddressAll');
			tripid.unsubscribeAddress(emitter1);
			tripid.unsubscribeAddressAll.callCount.should.equal(1);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			tripid.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			tripid.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			tripid.unsubscribeAddressAll(emitter1);
			tripid.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			tripid.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(tripid.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var tripid = new TripiService(baseConfig);
			var config = tripid._getDefaultConfig();
			config.should.equal(defaultTripiConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a tripi.conf file', function() {
			var TestTripi = proxyquire('../../lib/services/tripid', {
				fs: {
					readFileSync: readFileSync,
			...
			var tripid = new TestTripi(baseConfig);
			tripid.options.spawn.datadir = '/tmp/.tripi';

			sinon.spy(tripid, '_expandRelativeDatadir');
			sinon.spy(tripid, '_checkConfigIndexes');
			sinon.spy(tripid, '_getDefaultConf')
			sinon.spy(tripid, '_getNetworkConfigPath');
			sinon.spy(tripid, '_parseBitcoinConf');

			var node = {};
			tripid._loadSpawnConfiguration(node);

			tripid._expandRelativeDatadir.callCount.should.equal(1);
			tripid._checkConfigIndexes.callCount.should.equal(1);
			tripid._getDefaultConf.callCount.should.equal(1);
			tripid._getNetworkConfigPath.callCount.should.equal(1);
			tripid._parseBitcoinConf.callCount.should.equal(2);

			should.exist(tripid.spawn.config);
			tripid.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: tripicore.Networks.testnet,
					configPath: '/tmp/.tripicore/tripicore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var tripid = new TestTripi(config);
			...
			tripid.options.spawn.datadir = './data';
			var node = {};
			tripid._loadSpawnConfiguration(node);
			...
			tripid.options.spawn.datadir.should.equal('/tmp/.tripicore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestTripi = proxyquire('../../lib/services/tripid', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badtripi.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var tripid = new TestTripi(baseConfig);
			(function() {
				tripid._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(tripicore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the tripi.conf file', function() {
			var tripid = new TripiService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				tripid._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the tripi.conf file', function() {
			var tripid = new TripiService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the tripi.conf file', function() {
			var tripid = new TripiService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the tripi.conf file', function() {
			var tripid = new TripiService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the tripi.conf file', function() {
			var tripid = new TripiService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				tripid.transactionDetailedCache.set(keys[i], {});
				tripid.utxosCache.set(keys[i], {});
				tripid.accountInfo.set(keys[i], {});
				tripid.txidsCache.set(keys[i], {});
				tripid.balanceCache.set(keys[i], {});
				tripid.summaryCache.set(keys[i], {});
				tripid.blockOverviewCache.set(keys[i], {});
				tripid.blockJsonCache.set(keys[i], {});
			}
			tripid._resetCaches();
			should.equal(tripid.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(tripid.utxosCache.get(keys[0]), undefined);
			should.equal(tripid.accountInfo.get(keys[0]), undefined);
			should.equal(tripid.txidsCache.get(keys[0]), undefined);
			should.equal(tripid.balanceCache.get(keys[0]), undefined);
			should.equal(tripid.summaryCache.get(keys[0]), undefined);
			should.equal(tripid.blockOverviewCache.get(keys[0]), undefined);
			should.equal(tripid.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			tripid._tryAllClients(function(client, next) {
				client.getBlockChainInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				tripid.nodes[0].client.getBlockChainInfo.callCount.should.equal(1);
				tripid.nodes[1].client.getBlockChainInfo.callCount.should.equal(1);
				tripid.nodes[2].client.getBlockChainInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			tripid.nodes.push({
				client: {
					getBlockChainInfo: sinon.stub().callsArgWith(0, new Error('2'))
				}
			});
			tripid.nodes.push({
				client: {
					getBlockChainInfo: sinon.stub().callsArgWith(0, new Error('3'))
				}
			});
			tripid.nodes.push({
				client: {
					getBlockChainInfo: sinon.stub().callsArgWith(0, new Error('1'))
				}
			});
			tripid.nodesIndex = 2;
			tripid._tryAllClients(function(client, next) {
				...
				tripid.nodes[1].client.getBlockChainInfo.callCount.should.equal(1);
				tripid.nodes[2].client.getBlockChainInfo.callCount.should.equal(1);
				tripid.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert tripid-rpc error object into JavaScript error', function() {
			var tripid = new TripiService(baseConfig);
			var error = tripid._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			tripid.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			tripid.nodes.push({
				client: {
			...
			tripid._initChain(function() {
				log.info.callCount.should.equal(1);
				tripid.getRawBlock.callCount.should.equal(1);
				tripid.getRawBlock.args[0][0].should.equal('genesishash');
				tripid.height.should.equal(5000);
				tripid.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			tripid._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			tripid.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			tripid._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			tripicore.Networks.disableRegtest();
			baseConfig.node.network = tripicore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: tripicore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var tripid = new TripiService(config);
			tripid._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var tripid = new TripiService(config);
			tripid._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			tripicore.Networks.enableRegtest();
			var config = {
				node: {
					network: tripicore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var tripid = new TripiService(config);
			should.equal(tripid._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var tripid = new TripiService(config);
			tripid._getNetworkConfigPath().should.equal('testnet3/tripi.conf');
		});
		it('will get default rpc port for regtest', function() {
			tripicore.Networks.enableRegtest();
			var config = {
			...
			var tripid = new TripiService(config);
			tripid._getNetworkConfigPath().should.equal('regtest/tripi.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var tripid = new TripiService(baseConfig);
			tripid.node.network = tripicore.Networks.testnet;
			tripid._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var tripid = new TripiService(baseConfig);
			tripid.node.network = tripicore.Networks.testnet;
			tripicore.Networks.enableRegtest();
			tripid._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var tripid = new TripiService(baseConfig);
			tripid.node.network = tripicore.Networks.livenet;
			tripicore.Networks.enableRegtest();
			should.equal(tripid._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			tripid._rapidProtectedUpdateTip = sinon.stub();
			tripid.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			tripid._zmqBlockHandler(node, message);
			tripid._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			tripid._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(tripid, 'emit');
			tripid.on('block', function(block) {
				block.should.equal(message);
				tripid.emit.callCount.should.equal(1);
				tripid._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			tripid._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			tripid._zmqBlockHandler(node, message);
			tripid._rapidProtectedUpdateTip.callCount.should.equal(1);
			tripid._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			tripid._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(tripid, 'emit');
			var emitter = new EventEmitter();
			tripid.subscriptions.hashblock.push(emitter);
			emitter.on('tripid/hashblock', function(blockHash) {
				tripid._rapidProtectedUpdateTip.callCount.should.equal(1);
				tripid.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				tripid._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub();
			sinon.spy(tripid, '_resetCaches');
			tripid.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				tripid._resetCaches.callCount.should.equal(1);
			...
			tripid._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			tripid.on('synced', function() {
				tripid.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			tripid.on('synced', function() {
				tripid.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			tripid.on('error', function(err) {
				log.error.callCount.should.equal(1);
				tripid.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub();
			tripid._resetCaches = sinon.stub();
			tripid.on('tip', function(height) {
				tripid._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				tripid.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using tripicore.Transaction', function() {
			var tripid = new TripiService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = tripicore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(tripicore.Networks.testnet);
			var outputAddress = tripicore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = tripicore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: tripicore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = tripid._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(tripicore.Transaction.Output({
				script: tripicore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = tripid._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var tripid = new TripiService(baseConfig);
			var tx = tripicore.Transaction();
			var address = tripicore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(tripicore.Transaction.Output({
				script: tripicore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(tripicore.Transaction.Output({
				script: tripicore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			tripid._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			tripid.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('tripid/addresstxid', function(data) {
			...
			tripid._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			tripid.subscriptions.rawtransaction.push(emitter);
			emitter.on('tripid/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			tripid._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(tripid, 'emit');
			tripid.subscriptions.rawtransaction.push(emitter);
			emitter.on('tripid/rawtransaction', function() {
				done();
			});
			var node = {};
			tripid._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._updateTip = sinon.stub();
			tripid._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			tripid.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			tripid._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				tripid._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			tripid._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				tripid._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			tripid._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				tripid._zmqBlockHandler.callCount.should.equal(0);
				tripid._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var tripid = new TripiService(baseConfig);
			var node = {};
			tripid._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			tripid._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			tripid._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			tripid.on('tip', function(height) {
				...
				tripid.height.should.equal(100);
				done();
			});
			tripid._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestTripiService = proxyquire('../../lib/services/tripid', {
				fs: {
					readFile: readFile
				}
			});
			var tripid = new TestTripiService(baseConfig);
			tripid.spawnStopTime = 1;
			tripid._process = {};
			tripid._process.kill = sinon.stub();
			tripid._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				tripid._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			tripid._process.kill = sinon.stub().throws(error2);
			tripid._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._loadSpawnConfiguration = sinon.stub();
			tripid._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			tripid._spawnChildProcess(function(err) {
				tripid._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var tripid = new TripiService(baseConfig);
			tripid._loadSpawnConfiguration = sinon.stub();
			tripid._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			tripid._spawnChildProcess(function(err) {
				tripid._loadSpawnConfiguration.callCount.should.equal(1);
				tripid._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var tripid = new TestTripiService(config);
			tripid.spawn = {};
			tripid._loadSpawnConfiguration = sinon.stub();
			tripid._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			tripid.node.stopping = true;
			tripid._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			tripid._loadSpawnConfiguration = sinon.stub();
			tripid.spawn = {};
			tripid.spawn.exec = 'testexec';
			tripid.spawn.configPath = 'testdir/tripi.conf';
			tripid.spawn.datadir = 'testdir';
			tripid.spawn.config = {};
			tripid.spawn.config.rpcport = 20001;
			tripid.spawn.config.rpcuser = 'tripi';
			tripid.spawn.config.rpcpassword = 'password';
			tripid.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			tripid._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			tripid._initZmqSubSocket = sinon.stub();
			tripid._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tripid._checkReindex = sinon.stub().callsArgWith(1, null);
			tripid._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/tripi.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				tripid._loadTipFromNode.callCount.should.equal(1);
				tripid._initZmqSubSocket.callCount.should.equal(1);
				should.exist(tripid._initZmqSubSocket.args[0][0].client);
				tripid._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				tripid._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(tripid._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn tripid spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestTripiService = proxyquire('../../lib/services/tripid', {
				fs: {
			...
			tripid._loadSpawnConfiguration = sinon.stub();
			tripid.spawn = {};
			tripid.spawn.exec = 'tripid';
			tripid.spawn.datadir = '/tmp/tripi';
			tripid.spawn.configPath = '/tmp/tripi/tripi.conf';
			tripid.spawn.config = {};
			tripid.spawnRestartTime = 1;
			tripid._loadTipFromNode = sinon.stub().callsArg(1);
			tripid._initZmqSubSocket = sinon.stub();
			tripid._checkReindex = sinon.stub().callsArg(1);
			tripid._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tripid._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(tripid, '_spawnChildProcess');
			tripid._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						tripid._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			tripid._spawnChildProcess(function(err) {
				...
				tripid._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				tripid.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn tripid spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestTripiService = proxyquire('../../lib/services/tripid', {
				fs: {
			...
			tripid._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						tripid._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			tripid.spawn.config.rpcpassword = 'password';
			tripid.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			tripid._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			tripid._spawnChildProcess(function(err) {
				tripid._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			tripid._initZmqSubSocket = sinon.stub();
			tripid._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			tripid._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var tripid = new TripiService(config);
			tripid.node.stopping = true;
			tripid.startRetryInterval = 100;
			tripid._loadTipFromNode = sinon.stub();
			tripid._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to tripid/);
				tripid._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			tripid.startRetryInterval = 1;
			var config = {};
			tripid._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				tripid._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			tripid._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			tripid._connectProcess(config, function(err, node) {
				should.not.exist(err);
				tripid._loadTipFromNode.callCount.should.equal(1);
				tripid._initZmqSubSocket.callCount.should.equal(1);
				tripid._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.options = {};
			tripid.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Tripi configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			tripid.options = {
				connect: [
					{}
				]
			...
			tripid.start(function(err) {
				tripid._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var tripid = new TripiService(baseConfig);
			var node = {};
			tripid._initChain = sinon.stub().callsArg(0);
			tripid._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			tripid.options = {
				spawn: {}
			};
			tripid.start(function(err) {
				should.not.exist(err);
				tripid.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(tripid, '_spawnChildProcess');
			var nodes = [{}];
			tripid._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			tripid.options = {
				connect: [
					{}
				]
			...
			tripid.start(function(err) {
				should.not.exist(err);
				tripid._initChain.callCount.should.equal(1);
				tripid._connectProcess.callCount.should.equal(1);
				tripid.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			tripid.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				tripid.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			tripid.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			tripid.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			tripid.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var tripid = new TripiService(baseConfig);
			var args = tripid._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var tripid = new TripiService(baseConfig);
			var args = tripid._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_normalizeAddressArg');
			tripid.nodes.push({
				client: {
			...
			var options = {};
			tripid.getAddressBalance(address, options, function(err) {
				tripid._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			tripid.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				tripid.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					tripid._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			tripid.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = tripid._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = tripid._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = tripid._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = tripid._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				tripid._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			tripid.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(tripid, '_getHeightRangeQuery');
			sinon.spy(tripid, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tripid.getAddressTxids(address, options, function(err) {
				should.exist(err);
				tripid._getHeightRangeQuery.callCount.should.equal(1);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tripid.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				tripid._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			tripid.getAddressTxids(address, options, function(err, txids) {
				...
				tripid.getAddressTxids(address, options, function(err, txids) {
					...
					tripid._getHeightRangeQuery.callCount.should.equal(2);
					tripid._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			tripid.getAddressTxids(address, options, function(err, txids) {
				...
				tripid.getAddressTxids(address, options, function(err, txids) {
					...
					tripid._getHeightRangeQuery.callCount.should.equal(4);
					tripid._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			tripid.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				tripid.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					tripid.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						tripid._getHeightRangeQuery.callCount.should.equal(3);
						tripid._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var tripid = new TripiService(baseConfig);
			tripid.height = 10;
			var confirmations = tripid._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var tripid = new TripiService(baseConfig);
			var result = {};
			tripid._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var tripid = new TripiService(baseConfig);
			var result = {};
			tripid._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var tripid = new TripiService(baseConfig);
			var result = {};
			tripid._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var tripid = new TripiService(baseConfig);
			var result = {};
			tripid._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(tripid, '_getAddressDetailsForInput');
			sinon.spy(tripid, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = tripid._getAddressDetailsForTransaction(tx, addresses);
			...
			tripid._getAddressDetailsForInput.callCount.should.equal(1);
			tripid._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_getConfirmationsDetail');
			tripid.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			tripid.height = 300;
			var addresses = {};
			tripid._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			tripid._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				tripid.getDetailedTransaction.callCount.should.equal(1);
				tripid._getAddressDetailsForTransaction.callCount.should.equal(1);
				tripid._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var tripid = new TripiService(baseConfig);
			tripid.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			tripid._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				tripid.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from tripicore addresses', function() {
			var addresses = [
				tripicore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				tripicore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var tripid = new TripiService(baseConfig);
			var strings = tripid._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var tripid = new TripiService(baseConfig);
			(function() {
				tripid._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_normalizeAddressArg');
			tripid.maxAddressesQuery = 100;
			tripid.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getAddressStrings');
			tripid.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getAddressStrings');
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			tripid.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getAddressStrings');
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			tripid._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			tripid.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid._getAddressStrings.callCount.should.equal(1);
				tripid.getAddressTxids.callCount.should.equal(1);
				tripid._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getAddressStrings');
			sinon.spy(tripid, 'getAddressTxids');
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			tripid.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getAddressStrings');
			sinon.spy(tripid, '_paginateTxids');
			tripid._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(tripid, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			tripid.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid._getAddressStrings.callCount.should.equal(1);
				tripid.getAddressTxids.callCount.should.equal(1);
				tripid._getAddressDetailedTransaction.callCount.should.equal(2);
				tripid._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			tripid.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			tripid.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			tripid.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			tripid.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			tripid.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			tripid.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			tripid.getAddressSummary(address, options, function(err, summary) {
				tripid._paginateTxids.callCount.should.equal(1);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid.getAddressTxids.callCount.should.equal(1);
				tripid.getAddressBalance.callCount.should.equal(1);
				tripid._paginateTxids.args[0][1].should.equal(0);
				tripid._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			tripid.getAddressSummary(address, options, function(err, summary) {
				...
				tripid.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					tripid._paginateTxids.callCount.should.equal(0);
					tripid._normalizeAddressArg.callCount.should.equal(2);
					tripid.getAddressTxids.callCount.should.equal(1);
					tripid.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			tripid.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				tripid._paginateTxids.callCount.should.equal(1);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				tripid.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			tripid._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			tripid.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(tripid, '_tryAllClients');
			tripid.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			tripid.getRawBlock(blockhash, function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			tripid.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			tripid.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			tripid.getBlock(0, function(err) {
				tripid._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as tripicore object from height', function(done) {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			tripid.getBlock(0, function(err, block) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(tripicore.Block);
				done();
			});
		});
		it('will getblock as tripicore object', function(done) {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			tripid.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			tripid.getBlock(hash, function(err, block) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			tripid.getBlock(0, function(err, block) {
				...
				tripid.getBlock(0, function(err, block) {
					should.not.exist(err);
					tripid._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			tripid.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			tripid.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			tripid.getBlockHeader(10, function(err) {
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			tripid.getBlockHeader(blockhash, function(err) {
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			tripid.getBlockHeader(0, function(err) {
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			tripid.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				tripid._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			tripid.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			tripid._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				tripid._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			tripid._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			tripid._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_maybeGetBlockHash');
			tripid._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			tripid.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			tripid.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				tripid.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					tripid._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			tripid.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			tripid.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = tripicore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			tripid.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			tripid.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			tripid.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = tripicore.Transaction();
			(function() {
				tripid.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getRawTransaction('txid', function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			tripid.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getTransaction('txid', function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			tripid.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				tripid._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(tripicore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			tripid.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				tripid._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			tripid.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getBlockChainInfo and give result', function(done) {
			...
			tripid.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			tripid.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getBlockChainInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getBlockChainInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getBlockChainInfo and give result', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.node.getNetworkName = sinon.stub().returns('testnet');
			var getBlockChainInfo = sinon.stub().callsArgWith(0, null, {
				result: {
					"version": 140900,
			...
			tripid.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			tripid.nodes.push({
				client: {
					getBlockChainInfo: getBlockChainInfo,
				},
			...
			tripid.getBlockChainInfo(function(err, info) {
				...
				tripid.getSubsidy.callCount.should.equal(1);
				tripid.client.getBlockChainInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			tripid.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			tripid.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.spawn = {};
			tripid.spawn.process = new EventEmitter();
			tripid.spawn.process.kill = sinon.stub();
			tripid.stop(done);
			tripid.spawn.process.kill.callCount.should.equal(1);
			tripid.spawn.process.kill.args[0][0].should.equal('SIGINT');
			tripid.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			tripid.spawn.process.kill.callCount.should.equal(1);
			tripid.spawn.process.kill.args[0][0].should.equal('SIGINT');
			tripid.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var tripid = new TripiService(baseConfig);
			tripid.shutdownTimeout = 300;
			tripid.spawn = {};
			tripid.spawn.process = new EventEmitter();
			tripid.spawn.process.kill = sinon.stub();
			tripid.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_normalizeAddressArg');
			sinon.spy(tripid, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			tripid.nodes.push({
			...
			tripid.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				tripid._getBalanceFromMempool.callCount.should.equal(1);
				tripid._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			tripid.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			tripid.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			tripid.getJsonBlock(0, function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			tripid.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			tripid.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(2);
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			tripid.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
				tripid._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var tripid = new TripiService(baseConfig);
			var emitter = new EventEmitter();
			tripid.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(tripid.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var tripid = new TripiService(baseConfig);
			var emitter = new EventEmitter();
			tripid.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			tripid.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			tripid.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeBalance(emitter1);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			tripid.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var tripid = new TripiService(baseConfig);
			...
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			tripid.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var tripid = new TripiService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(tripid, 'unsubscribeBalanceAll');
			tripid.unsubscribeBalance(emitter1);
			tripid.unsubscribeBalanceAll.callCount.should.equal(1);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			tripid.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			tripid.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			tripid.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			tripid.unsubscribeBalanceAll(emitter1);
			tripid.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			tripid.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			tripid.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(tripid.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var tripid = new TripiService(baseConfig);
			sinon.spy(tripid, '_notifyBalanceSubscriber');
			tripid._getAddressesFromTransaction = sinon.stub().returns(addresses);
			tripid.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('tripid/addressbalance', function(data) {
				...
				tripid._notifyBalanceSubscriber.callCount.should.equal(1);
				tripid._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			tripid._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('tripid/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				tripid.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			tripid._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			tripid.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			tripid.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			tripid.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getDgpInfo(function(err, result) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getMiningInfo(function(err, result) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			tripid.getStakingInfo(function(err, result) {
				should.exist(err);
				tripid._tryAllClients.callCount.should.equal(1);
